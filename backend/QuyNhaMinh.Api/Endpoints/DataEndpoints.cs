...
public static class DataEndpoints {
    ...
    static async Task<IResult> Excel(Guid fundId, DateTime from, DateTime to, FundAccess access, AppDb db) {
        await access.Require(fundId); var rows = await ReportRows(fundId, from, to, db);
    ...
    static async Task<IResult> Pdf(Guid fundId, DateTime from, DateTime to, FundAccess access, AppDb db) {
        await access.Require(fundId); var rows = await ReportRows(fundId, from, to, db); var income = rows.Where(x => x.Type == TransactionTypes.Income).Sum(x => x.Amount); var expense = rows.Where(x => x.Type == TransactionTypes.Expense).Sum(x => x.Amount);
    ...
    static Task<List<ReportRow>> ReportRows(Guid fundId, DateTime from, DateTime to, AppDb db) => db.Transactions.Where(t => t.FundId == fundId && t.TransactionDate >= from && t.TransactionDate < to).OrderByDescending(t => t.TransactionDate).Join(db.Categories, t => t.CategoryId, c => c.Id, (t, c) => new { t, c }).Join(db.MoneyAccounts, x => x.t.AccountId, a => a.Id, (x, a) => new { x.t, Category = x.c.Name, Account = a.Name }).Join(db.Users, x => x.t.CreatedBy, u => u.Id, (x, u) => new ReportRow(x.t.TransactionDate, x.t.Type, x.Category, x.Account, x.t.Amount, u.DisplayName, x.t.Merchant, x.t.Note)).ToListAsync();
